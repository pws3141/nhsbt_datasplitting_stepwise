...
Now `r iters` models have been built, let's look at the hazard ratio estimates. Each dot in @fig-full-coefficients is one coefficient estimate. We can compare the spread of these estimates to the confidence intervals given in @fig-single-hr -- we see that they are similar. However, when the point estimate is the element that is used (for example, in donor risk indices), it is important to show how the particular choice of coefficient estimate affects the DRI.
...
The associated C-statistics calculated on the test observation and given in @fig-full-cstats
...
First, create a data.table where each row consists of a model `.id`, and then the `r floor(n1 * n_x)` indices which are included in the `.id`$^{th}$ training set.
...
The C-statistics calcualted on the test sets associated with the `r iters` different models are shown in @fig-stepwise-cstats.
...
Choosing predictors based on literature review and expert knowledge is recommended in @efthimiou2024developing, where they state:
