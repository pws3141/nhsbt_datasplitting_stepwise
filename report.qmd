...
Now `r iters` models have been build, let's look at the coefficient estimates. Each dot in @fig-full-coefficients is one coefficient estimate.
...
```{r}
#| fig-cap: Strip plot showing the coefficient estimates from models built on the `r iter` different training sets.
